...
def _hostname(value: str) -> str:
    ...
    labels = body.split(".")
    if not body or any(not re.fullmatch(r"[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?", x, re.ASCII) for x in labels):
        raise TargetError("invalid hostname")
    return value
...
def assess_validity(not_before: datetime, not_after: datetime, warn_days: int,
                    clock: Callable[[], datetime] = lambda: datetime.now(timezone.utc)) -> ValidityAssessment:
    ...
    return ValidityAssessment(ValidityStatus.WARNING if warning else ValidityStatus.NORMAL, remaining, warning, 1 if warning else 0)
...
def resolve_candidates(target: Target, resolver=socket.getaddrinfo) -> list[ConnectionCandidate]:
    ...
    candidates = [ConnectionCandidate(r[0], r[1], r[2], r[4]) for r in records if r[1] == socket.SOCK_STREAM]
    if not candidates:
...
def observe_leaf(target: Target, resolver=socket.getaddrinfo, socket_factory=socket.socket,
                 context_factory=lambda: ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)) -> LeafObservation:
...
OPENSSL_ARGS = ("x509", "-inform", "DER", "-noout", "-subject", "-issuer", "-serial",
                "-startdate", "-enddate", "-ext", "subjectAltName", "-nameopt", "RFC2253")
...
def run_decoder(path: str, der: bytes, popen=subprocess.Popen) -> bytes:
    env = os.environ.copy(); env.update({"LC_ALL": "C", "LANG": "C"})
    try:
        proc = popen([path, *OPENSSL_ARGS], stdin=subprocess.PIPE, stdout=subprocess.PIPE,
                     stderr=subprocess.PIPE, env=env)
    except OSError as exc:
        raise CertWatchError("could not execute certificate decoder") from exc
    try:
        assert proc.stdin and proc.stdout and proc.stderr
        proc.stdin.write(der); proc.stdin.close()
...
def _parse_time(value: str) -> datetime:
    if not re.fullmatch(r"[A-Z][a-z]{2} {1,2}[0-9]{1,2} [0-9]{2}:[0-9]{2}:[0-9]{2} [0-9]{4} GMT", value):
        raise ValueError
    return datetime.strptime(value, "%b %d %H:%M:%S %Y GMT").replace(tzinfo=timezone.utc)


def _split_sans(text: str) -> tuple[tuple[str, str], ...]:
    # OpenSSL escapes commas inside supported IA5 values with a backslash. Split only unescaped commas.
...
def parse_certificate_output(raw: bytes, der: bytes) -> CertificateInfo:
    ...
    try:
        ...
        san_text = None
        index = 0
        prefixes = {"subject=": "subject", "issuer=": "issuer", "serial=": "serial",
                    "notBefore=": "not_before", "notAfter=": "not_after"}
        while index < len(lines):
            line = lines[index]
            found = next(((prefix, key) for prefix, key in prefixes.items() if line.startswith(prefix)), None)
            if found:
                prefix, key = found
                if key in fields: raise ValueError
                fields[key] = line[len(prefix):]
            elif line == "X509v3 Subject Alternative Name:":
                if san_text is not None or index + 1 >= len(lines): raise ValueError
                index += 1; san_text = lines[index].strip()
            elif line.strip():
                raise ValueError
            index += 1
        if set(fields) != {"subject", "issuer", "serial", "not_before", "not_after"}: raise ValueError
        if not fields["subject"] or not fields["issuer"]: raise ValueError
        if not re.fullmatch(r"[0-9A-Fa-f]+", fields["serial"]): raise ValueError
        before, after = _parse_time(fields["not_before"]), _parse_time(fields["not_after"])
        if after < before:
            raise CertWatchError("certificate contains unusable validity fields")
        sans = () if san_text is None else _split_sans(san_text)
        return CertificateInfo(fields["subject"], fields["issuer"], fields["serial"].upper(), sans,
                               before, after, fingerprint(der))
...
def render_report(target: Target, observation: LeafObservation, cert: CertificateInfo,
                  assessment: ValidityAssessment) -> str:
    sans = ", ".join(f"{kind}:{sanitize(value)}" for kind, value in cert.sans) or "-"
    if assessment.status is ValidityStatus.NORMAL:
        words = ["The presented leaf certificate is currently within its encoded validity period.",
                 "It is outside the configured expiration warning window."]
    elif assessment.status is ValidityStatus.WARNING:
        words = ["The presented leaf certificate is currently within its encoded validity period but is inside the configured expiration warning window."]
    ...
    return "\n".join([
        f"CertWatch: {sanitize(target.display_endpoint)}", "Scope: leaf TLS certificate presented by the selected endpoint", "", "Target",
        f"  Requested:          {sanitize(target.display_endpoint)}", f"  Connected address:  {sanitize(observation.connected_address)}", "", "Certificate",
        f"  Subject:       {sanitize(cert.subject)}", f"  Issuer:        {sanitize(cert.issuer)}", f"  Serial:        {cert.serial}",
        f"  SHA256:        {cert.sha256_fingerprint}", f"  Not before:    {cert.not_before:%Y-%m-%dT%H:%M:%SZ}",
        f"  Not after:     {cert.not_after:%Y-%m-%dT%H:%M:%SZ}", f"  SANs:          {sans}", "", "Validity",
        f"  Status:        {assessment.status.value}", f"  Remaining:     {_remaining(assessment.remaining)}", "", "Assessment",
...
def build_parser() -> argparse.ArgumentParser:
    parser = Parser(prog="certwatch", description="Observe one remote TLS leaf certificate and assess its encoded validity period.")
    parser.add_argument("target", help="HOST, HOST:PORT, bare IPv6, or [IPv6]:PORT")
    parser.add_argument("--warn-days", default="30", metavar="N", help="non-negative expiration warning threshold (default: 30)")
    return parser
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    try:
        ...
        try:
            ...
            print(f"certwatch: {sanitize(exc)}", file=sys.stderr); return 2
        ...
        print("certwatch: interrupted", file=sys.stderr); return 130
    except CertWatchError as exc:
        print(f"certwatch: {sanitize(exc)}", file=sys.stderr); return 3
    except Exception:
        print("certwatch: internal execution failure", file=sys.stderr); return 3
